...
`kanji.db` ships **all 2136 Jōyō kanji** + stroke SVGs + ~15k vocab (~13 MB).
Charset files in `resources/jlpt/`: `joyo.txt` (build scope) and `n1/n2/n4/n5.txt`
(JLPT tags). `n5.txt` is hand-curated; `joyo/n1/n2/n4` are regenerated by
`scripts.make_charsets` (approximate JLPT from KANJIDIC legacy levels; no N3).
